    ...
    OpenSshGuiDbContext dbContext) : ViewModelBase<ApplicationSettingsViewModel>(logger)
{
    ...
    private List<IConnectionCredentials> _knownServers = new();

    private Settings _settings;
    public Interaction<EditSavedServerEntryViewModel, EditSavedServerEntryViewModel?> ShowEditEntry = new();

    public ObservableCollection<SshKeyFile> Keys => locatorService.SshKeys;
    ...
    public ReactiveCommand<IConnectionCredentials, IConnectionCredentials?> EditEntry =>
        ReactiveCommand.CreateFromTask<IConnectionCredentials, IConnectionCredentials?>(async e =>
        {
    ...
    public override async ValueTask InitializeAsync(
        IInitializerParameters<ApplicationSettingsViewModel>? initializerParameters = null,
        CancellationToken cancellationToken = default)
    {
        _settings = dbContext.Settings.First();
        _convertPpkAutomatically = _settings.ConvertPpkAutomatically;
        foreach (var connectionCredentialsDto in dbContext.ConnectionCredentialsDtos)
            _knownServers.Add(await connectionCredentialsDto.ToCredentials());
        BooleanSubmit = ReactiveCommand.CreateFromTask<bool, ApplicationSettingsViewModel?>(async e =>
        {
            ...
            await context.ConnectionCredentialsDtos.Where(f => !knownServerIds.Contains(f.Id))
