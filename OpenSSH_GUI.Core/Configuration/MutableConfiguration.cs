    ...
    where T : class
{
    ...
    private void SetPropertyValue<TValue>(PropertyInfo propertyInfo, T config, TValue value)
    {
        ...
        _logger.LogDebug(
            "Updated property {PropertyName} of configuration object '{ConfigurationType}' from {InitialValue} to {CurrentValue}", propertyInfo.Name, typeof(T).Name, initialValue,
            value);
    }
