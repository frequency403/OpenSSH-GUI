...
public partial record SshKeyFileInformation : ReactiveRecord
{
    ...
    /// </summary>
    [Reactive(SetModifier = AccessModifier.Private)] private FileInfo _fileInfo = new(Assembly.GetExecutingAssembly().Location);
    [Reactive(SetModifier = AccessModifier.Private)] private SshKeyFileSource _keyFileSource;
    [Reactive(SetModifier = AccessModifier.Private)] private bool _canChangeFileName;
    ...
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private SshKeyFormat _currentFormat = SshKeyFormat.OpenSSH;
    ...
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private bool _isOpenSshKey;
    ...
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private string _fileName = string.Empty;
    ...
    /// </value>
    [Reactive(SetModifier = AccessModifier.Private)] private string? _publicKeyFileName;
    ...
    ///     the filesystem.
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private string _fullFileName = string.Empty;
    ...
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private bool _exists;
    ...
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private string? _directoryName;

    /// <summary>
    ///     Represents a collection of key-related files associated with an SSH key.
    /// </summary>
    [Reactive(SetModifier = AccessModifier.Private)] private FileInfo[] _files = [];
    ...
    ///     OpenSSH and PuTTYv3.
    /// </remarks>
    [Reactive(SetModifier = AccessModifier.Private)] private SshKeyFormat[] _availableFormatsForConversion = [];
    ...
    [Reactive(SetModifier = AccessModifier.Private)] private SshKeyFormat _defaultConversionFormat = SshKeyFormat.OpenSSH;

    public SshKeyFileInformation(SshKeyFileSource keyFileSource)
    {
        KeyFileSource = keyFileSource;
        this.WhenAnyValue(vm => vm.KeyFileSource)
            .ObserveOn(AvaloniaScheduler.Instance)
            .Subscribe(fileSource =>
            {
                CanChangeFileName = !fileSource.ProvidedByConfig;
                FileInfo = new FileInfo(fileSource.AbsolutePath);
            })
            .DisposeWith(_disposables);

        this.WhenAnyValue(vm => vm.FileInfo)
            .ObserveOn(AvaloniaScheduler.Instance)
            .Subscribe(info =>
            {
                CurrentFormat = info.Extension switch
                {
        ...
        this.WhenAnyValue(vm => vm.CurrentFormat)
            .ObserveOn(AvaloniaScheduler.Instance)
            .Subscribe(format =>
            {
                IsOpenSshKey = format is SshKeyFormat.OpenSSH;
                PublicKeyFileName = IsOpenSshKey ? Path.ChangeExtension(FileInfo.FullName, "pub") : null;
                AvailableFormatsForConversion = AvailableFormats.Where(e => e != format).ToArray();
                DefaultConversionFormat = AvailableFormatsForConversion.Contains(SshKeyFormat.OpenSSH)
                    ? SshKeyFormat.OpenSSH
                    : AvailableFormatsForConversion.OrderDescending().FirstOrDefault(SshKeyFormat.OpenSSH);
            })
            .DisposeWith(_disposables);

        this.WhenAnyValue(vm => vm.FullFileName, vm => vm.PublicKeyFileName)
            .ObserveOn(AvaloniaScheduler.Instance)
            .Select(tuple => new[] { tuple.Item1, tuple.Item2 }.Where(e => !string.IsNullOrEmpty(e))
                .Select(e => new FileInfo(e!)).ToArray())
            .Subscribe(fileInfoArray =>
            {
                Files = fileInfoArray;
            }).DisposeWith(_disposables);
    }
