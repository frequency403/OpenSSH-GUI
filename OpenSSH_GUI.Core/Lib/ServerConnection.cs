...
public class ServerConnection : ReactiveObject, IServerConnection
{
    ...
    private ServerConnection(IConnectionCredentials credentials)
    {
    ...
    public ServerConnection(string hostname, string user, IEnumerable<ISshKey> keys) : this(
        new MultiKeyConnectionCredentials
        {
            Hostname = hostname,
            Username = user,
            Keys = keys
    ...
    public string ConnectionString =>
        IsConnected ? $"{_connectionCredentials.Username}@{_connectionCredentials.Hostname}" : "";
